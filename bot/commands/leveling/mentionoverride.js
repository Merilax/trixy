...
module.exports.run = async (
    ...
    if (args[0].trim().match(/^(true|false|reset)$/i)) {
        if (args[0] === "true") {
            await db.guildLevelConfigDB.updateOne({ doMention: 1 }, { where: { guildId: message.guild.id } });
            ...
            await db.guildLevelConfigDB.updateOne({ doMention: 0 }, { where: { guildId: message.guild.id } });
            ...
            await db.guildLevelConfigDB.updateOne({ doMention: null }, { where: { guildId: message.guild.id } });
        }
    } else {
