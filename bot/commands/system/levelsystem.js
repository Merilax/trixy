...
module.exports.run = async (
    ...
    if (message.author.id !== message.guild.ownerId) {
        ...
        if (args[0] === "enable") {
            await db.guildConfigDB.update({ xpEnabled: true }, { where: { guildId: message.guild.id } });
            return message.channel.send({ content: `${TxTE.emoji.ok} **Enabled** leveling system!` });
        } else if (args[0] === "disable") {
            await db.guildConfigDB.update({ xpEnabled: false }, { where: { guildId: message.guild.id } });
