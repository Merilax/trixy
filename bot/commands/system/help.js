...
module.exports.run = (
    ...
    if (args[0]) {
        ...
        if (help) {
            ...
            return message.channel.send({ content: `${TxTE.emoji.x} That does not look like a command I would know. Try not to use aliases.` });
        }
    } else {
        var helpembed = new Discord.EmbedBuilder()
            .setTitle(`${TxTE.emoji.settings} **I'd love to help! Listing all commands:**`)
            .setDescription("Need help? [Join the support server!](https://discord.gg/ZbUQ8Mh)\nPut a command after `Trixy, help` for more info about it.\nHave you seen our new website? Check it out [here](https://trixy-v2.herokuapp.com)!")
            .addFields([
                {
