...
module.exports.run = async (
    ...
    if (message.author.id !== message.guild.ownerId)
        return message.channel.send({ content: `${TxTE.emoji.block} Only the server owner may change that.` });

    const [xpEnable, xpCreated] = await db.guildConfigDB.findOrCreate({
        where: { guildId: message.guild.id },
        defaults: { guildId: message.guild.id }
    });

    if (xpEnable.xpEnabled === false)
        return message.channel.send({ content: `${TxTE.emoji.x} Level system must be enabled.` });

    const newColour = args[0].trim();
    if (newColour.match(/^#[0-9a-f]{3,6}$/i)) {
        try {
            const [guildColour, created] = await db.guildConfigDB.findOrCreate({
                where: { guildId: message.guild.id },
                defaults: { color: newColour }
            });

            if (!created) {
                if (newColour === "#000000" || newColour === "#000") {
                    await db.guildConfigDB.update({ color: null }, {
                        where: { guildId: message.guild.id }
                    });
                    message.channel.send({ content: `${TxTE.emoji.ok} Custom colour disabled. Try #001 if you wanted black.` });
                } else {
                    await db.guildConfigDB.update({ color: newColour }, {
                        where: { guildId: message.guild.id }
                    });
