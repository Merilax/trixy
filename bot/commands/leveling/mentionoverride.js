...
module.exports.run = async (
    ...
    if (args[0].trim().match(/^(true|false|reset)$/i)) {
        if (args[0] === "true") {
            await db.guildLevelConfigDB.update({ doMention: 1 }, {
            ...
            await db.guildLevelConfigDB.update({ doMention: 0 }, {
                where: { guildId: message.guild.id }
            });
            ...
            await db.guildLevelConfigDB.update({ doMention: null }, {
                where: { guildId: message.guild.id }
            });
            message.channel.send({ content: `${TxTE.emoji.ok} ${TxTE.affirmation[Math.floor(Math.random() * TxTE.affirmation.length)]} No longer overriding guild configurations.` });
        }
    } else {
        message.channel.send({ content: `${TxTE.emoji.quote} Please specify \`true\` if you want to be mentioned upon levelling up, or \`false\` otherwise. If you want the guild to decide, append \`reset\`.` });
    }
};